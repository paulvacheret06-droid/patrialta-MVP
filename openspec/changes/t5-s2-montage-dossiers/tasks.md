...
- [ ] 2.6 Créer le bucket Supabase Storage `dossiers-pieces` en mode privé (pas d'accès public) via Supabase Dashboard, configurer la policy d'accès (authenticated uniquement, owner = auth.uid())
...
- [ ] 11.6 Tester manuellement la route `/api/aides/sync` en local avec le header `Authorization: Bearer <CRON_SECRET>` et vérifier les aides dans Supabase
