...
Le projet MUST fournir un fichier `/lib/s1/seed/aides.ts` exportant un tableau `Aide[]` d'au moins 10 aides couvrant : au moins 1 aide DRAC nationale, 1 aide région AuRA, 1 aide Fondation du Patrimoine, 1 aide fondation privée (VMF ou Sauvegarde de l'Art Français), 1 aide européenne (FEDER ou LEADER). Un script `scripts/seed-aides.ts` MUST insérer ces données via le client `service_role`.

#### Scenario: Seed exécuté sans erreur
- **WHEN** `npx tsx scripts/seed-aides.ts` est exécuté sur une base vide
- **THEN** exactement 10 lignes sont insérées dans `aides` sans erreur
